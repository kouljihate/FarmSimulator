...
92. v0.47.0: **Sectors row 3-column layout** — `_sectors_result.html` changed the sector count/checkbox row from a single `flex` row to a `grid grid-cols-3` layout: col 1 (15%) shows "N sectors" (EN) | col 2 (70%) shows sector checkboxes | col 3 (15%, RTL) shows "قطاعات N" (AR).