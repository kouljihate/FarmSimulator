...
91. v0.47.0: **_existing_config now filters to description='Sector' only** — in `core/engine.py`, `_existing_config()` previously kept ALL extra polygons from the KML (regardless of description), causing many spurious sectors. Now filters to only polygons whose description equals `sector` (case-insensitive). If no such polygons exist, returns `None` and the engine falls back to auto sectorisation via `sectorise()`.