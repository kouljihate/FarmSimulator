...
90. v0.47.0: **Sector polygon duplicate fix** — fixed `_clean_slivers()` in `core/sector.py`: `unary_union()` can return a `MultiPolygon` or `GeometryCollection` when floating-point prevents merging tiny slivers into their nearest neighbor; now takes the largest polygon part (discarding duplicates). Added centroid dedup at the end of `_clean_slivers()` to prevent overlapping cells from surviving. Added overlap-area dedup in `partition()`: sorted by area descending, removes any cell whose overlap with a kept cell exceeds 10%% of its area. Fixed `_existing_config()` in `core/engine.py`: all sectors are now uniformly named `S1`, `S2`, ... regardless of KML polygon description, so no more `Basin 1` / `B1` names.
91. v0.47.0: **_existing_config reads all KML polygons** — removed description-based filtering; all non-land, non-water polygons from the KML are now kept as existing sectors. The sector count depends on what's actually in the file.
92. v0.47.0: **Sectors row 3-column layout** — `_sectors_result.html` changed the sector count/checkbox row from a single `flex` row to `grid grid-cols-[15%_70%_15%]`: col 1 (15%, dir="ltr") shows "N sectors" | col 2 (70%) shows sector checkboxes | col 3 (15%, dir="rtl") shows "قطاعات N".