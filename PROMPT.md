...
91. v0.47.0: **_existing_config filters to 'sector' description** — in `core/engine.py`, `_existing_config()` previously kept ALL extra polygons from the KML regardless of description. Now filters to only polygons whose description contains "sector" as a substring (case-insensitive), excluding "Water", "Land", "Basin" etc. If no such polygons exist, returns `None` and the engine falls back to auto sectorisation via `sectorise()`.